...
class PGFast {
  ...
  generateSubString = () => {
    ...
    randomNum >= randomWord.length - 3 ? subString = randomWord.slice(randomWord.length - 3) : subString = randomWord.slice(randomNum, randomNum + 3);
    return subString;
  }
  ...
  playGame(message, players) {
    ...
    const msgCollector = message.channel.createMessageCollector(() => true, { time: 10000 });
    ...
    msgCollector.on("collect", m => {
      ...
      if (!m.author.bot && Object.keys(players).includes(tag) && !firstValidWord && msg.includes(subString) && this.dictionary.check(msg)) {
        m.react("✅");
        players[tag] += 1;
        firstValidWord = true;
      }
...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (gameRunning) return message.channel.send("Finish the previous game first.");
      ...
      gameRunning = true;
      message.channel.send("```Game will start in 15 seconds. Click on the checkmark to participate. Be the fastest to write a word containing the group of 3 letters indicated.```")
        .then(message => {
          message.react("✅");
